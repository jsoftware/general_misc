...
if. #emsgs do.
  ...
  grid ('Errors found in';sourcefn) , ((1&{"1 emsgs) (0&{::"1 emsgs)} (#lines) # <'') ,. lines
else.
...
for_l. |. }. qname do.
  ...
  loc =. ". loc   NB. Convert string form of value to real form
...
NB. Class: /   u is AR of the actual conjunction used
NB. v is 0 to use monadic flags only, 1 to use both
lintaclass0 =: 2 : 0
typeu =. 0 {:: y
...
NB. For noun, we produce a side-effectful unknown, since we aren't into analyzing gerunds (or names in the case of f., or indices for })
if. noun bwand typeu do.
  (flags bwor verb+sideeff);''
